...
(() => {
  ...
  async function refreshWorkspace() {
    ...
    try {
      const listing = await api("/api/workspace?limit=200");
      const entries = listing.entries || [];
      if (!entries.length) {
        body.innerHTML =
          '<p class="muted">Workspace is empty. Run a mission or council to create artifacts under <code>reports/</code>, <code>plans/</code>, and <code>council/</code>.</p>';
        return;
      }
      const rows = entries
        .map(
          (e) =>
            `<tr data-path="${esc(e.path)}" class="ws-row"><td class="ws-path">${esc(
              e.path
            )}</td><td class="ws-size">${esc(e.size)}</td></tr>`
        )
        .join("");
      body.innerHTML = `<table class="ws-table"><thead><tr><th>Path</th><th>Bytes</th></tr></thead><tbody>${rows}</tbody></table><pre class="ws-preview muted" id="ws-preview">Click a file to preview.</pre>`;
      body.querySelectorAll("tr.ws-row").forEach((tr) => {
        tr.addEventListener("click", async () => {
          const path = tr.dataset.path;
          const prev = $("#ws-preview");
          prev.textContent = "Loading " + path + "…";
          try {
            const f = await api(
              "/api/workspace/file?path=" + encodeURIComponent(path) + "&max_chars=12000"
            );
            prev.textContent =
              `# ${f.path} (${f.size} bytes)` +
            ...
            prev.textContent = "Error: " + err.message;
          }
        });
      });
    } catch (e) {
      body.textContent = "Workspace API error: " + e.message;
    }
  }
